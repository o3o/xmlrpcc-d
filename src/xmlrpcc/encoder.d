...
Element encodePrimitiveValue(Variant param) {
   ...
   if (param.convertsTo!(const(string))() || param.convertsTo!(const(dstring))() || param.convertsTo!(const(wstring))()) {
...
version (xmlrpc_unittest) unittest {
   import xmlrpcc.decoder;

   static import xmlrpcc.paramconv;
   ...
   Variant[] params = [
      Variant(123U), Variant(cast(const) "Our sun is dying."), Variant(0xc0a1_e5ce_d64b_17UL), Variant([Variant(123),
      Variant(12.3), Variant("abc"d), Variant(0), Variant(true), Variant(null)
   ]), Variant(DateTime(2013, 8, 25, 13, 38, 42)), Variant(cast(const(ubyte[])) x"de ad be ef"),
      Variant(["null" : Variant(null)]), Variant(["true" : Variant(true)]), Variant(["false" : Variant(false)]),
   ...
   /*
     * Fault response
     */
   auto faultParams = xmlrpcc.paramconv.paramsToVariantArray(["faultCode"w : Variant(42),
