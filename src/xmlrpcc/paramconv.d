...
import std.traits : isAssociativeArray, isArray, isImplicitlyConvertible, KeyType, ValueType, isSomeString, isScalarType, Unqual;
...
auto variantArrayToParams(Args...)(Variant[] variants) {
   enforce(Args.length == variants.length,
         new ParameterConversionException(format("Wrong number of arguments: expected %s, got %s)", Args.length,
               variants.length)));
...
Variant paramToVariant(Arg)(Arg arg) {
   ...
   } else static if (isForwardRange!Arg) // Order matters because strings are forward ranges too.
   {
      ...
      static assert(isSomeString!(KeyType!Arg) || isImplicitlyConvertible!(KeyType!Arg, const(string))
